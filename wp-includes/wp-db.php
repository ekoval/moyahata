...
class wpdb {
	...
	function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Неправильний префікс БД'/*/WP_I18N_DB_BAD_PREFIX*/);
	...
	function select( $db, $dbh = null) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( /*WP_I18N_DB_SELECT_DB*/'<h1>Не вдалося вибрати базу даних</h1>
...
<p>Якщо Ви не знаєте, як налаштувати доступ до бази даних, слід <strong> звернутися до хостинґ-провайдера </strong>.Якщо проблему так і не вдалося вирішити - можливо, вам допоможуть на <a href="http://wordpress.co.ua/support/">Форумі підтримки WordPress</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser ), 'db_select_fail' );
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'Помилка БД WordPress %1$s для запиту %2$s спричинена %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'Помилка БД WordPress  %1$s для запиту%2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
...
'/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Тип виводу має бути одним з: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
